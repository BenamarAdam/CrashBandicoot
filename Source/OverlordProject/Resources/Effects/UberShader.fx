...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	if(gUseTextureNormal)
	{
		...
		float3 texSample = gTextureNormal.Sample(gTextureSampler, texCoord);
		
		texSample.x = 2 * texSample.x - 1;
		texSample.y = 2 * texSample.y - 1;
		texSample.z = 2 * texSample.z - 1;
		
		newNormal = mul(texSample,(float3x3)localAxis);
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	{		
		...
		fresnel = saturate(pow(fresnel, gFresnelPower));
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	if(gUseEnvironmentMapping)
	{
		...
		float3 reflectionColor = gCubeEnvironment.Sample(gTextureSampler,reflectionVector);
		reflectionColor *= gReflectionStrength;
		
		float3 refractionVector = refract(viewDirection, -normal, gRefractionIndex);
		float3 refractionColor = gCubeEnvironment.Sample(gTextureSampler,refractionVector);
		refractionColor *= gRefractionStrength;
		
		environmentColor = refractionColor * reflectionColor;
	}
