...
void DeathScreen::Initialize(const SceneContext&)//sceneContext)
{
	...
	auto sprite = obj->AddComponent(new SpriteComponent(L"Game/Textures/Menu/death.png", { 0.5f,0.5f }, { 1.f,1.f,1.f,1.f }));
...
void DeathScreen::Update(const SceneContext& sceneContext)
{
	...
	if (pInput->IsGamepadButton(InputState::pressed, XINPUT_GAMEPAD_A) || pInput->IsKeyboardKey(InputState::pressed, VK_SPACE))
	{
		...
		{
		case restart:
			dynamic_cast<DeathScene*>(GetScene())->SwitchToGame();			
			return;
		case menu:
