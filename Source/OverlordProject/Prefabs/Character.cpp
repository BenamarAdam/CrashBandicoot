...
{
	if (m_pCameraComponent->IsActive()||true)
	{
		...
		if (pInput->IsMouseButton(InputState::down, 1))
		{
		...
		m_TotalYaw += 0 * time * m_CharacterDesc.rotationSpeed;
		m_TotalPitch += 0 * time * m_CharacterDesc.rotationSpeed;
		//m_TotalYaw += look.x * time * m_CharacterDesc.rotationSpeed;
		//m_TotalPitch += look.y * time * m_CharacterDesc.rotationSpeed;
		//Rotate this character based on the TotalPitch (X) and TotalYaw (Y)
		pTransform->Rotate(m_TotalYaw, m_TotalPitch, 0);
		//********
		//MOVEMENT
