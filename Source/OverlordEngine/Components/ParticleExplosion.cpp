...
#include "../../OverlordProject/Scenes/Game/CrashGameScene.h"
#include "Misc/ParticleMaterial.h"

class CrashGameScene;
ParticleMaterial* ParticleExplosion::m_pParticleMaterial{};

ParticleExplosion::ParticleExplosion(const std::wstring& assetFile, const veloParticleEmitterSettings& emitterSettings, UINT particleCount) :
	m_ParticlesArray(new veloParticle[particleCount]),
...
void ParticleExplosion::Initialize(const SceneContext& sceneContext)
{
	...
	{
		m_ParticlesArray[i].direction = XMFLOAT3(((float)rand()-RAND_MAX/2) / RAND_MAX * 10, ((float)rand() - RAND_MAX / 2) / RAND_MAX * 10, ((float)rand() - RAND_MAX / 2) / RAND_MAX * 10);
	}
	CreateVertexBuffer(sceneContext);
	m_pParticleTexture = ContentManager::Load<TextureData>(m_AssetFile);
}

void ParticleExplosion::CreateVertexBuffer(const SceneContext& sceneContext)
{
...
void ParticleExplosion::Update(const SceneContext& sceneContext)
{
	TODO_W9(L"Implement Update")
		...
		if (!m_IsPaused)
		{
			...
			if (m_EmittedTime > m_EmitterSettings.timeToEmit)
			{
				SafeDelete(m_ParticlesArray);
				SafeRelease(m_pVertexBuffer);
				dynamic_cast<CrashGameScene*>(GetScene())->RemoveObject(GetGameObject());
				return;
			}
		}
	...
	{
		veloParticle& particle = m_ParticlesArray[i];
		bool active = false;
...
void ParticleExplosion::UpdateParticle(veloParticle& p, float elapsedTime) const
{
	TODO_W9(L"Implement UpdateParticle")
		if (!m_IsPaused)
		{
			...
			p.direction.y -= 9.81f * elapsedTime;
			...
			pos += XMLoadFloat3(&p.direction) * elapsedTime;
...
void ParticleExplosion::SpawnParticle(veloParticle& p)
{
...
void ParticleExplosion::PostDraw(const SceneContext& sceneContext)
{
...
void ParticleExplosion::DrawImGui()
{
