...
void ControllerComponent::Initialize(const SceneContext& /*sceneContext*/)
{
	if(!m_IsInitialized)
	{
		...
		XMFLOAT3 pos = m_pGameObject->GetComponent<TransformComponent>()->GetPosition();
		m_ControllerDesc.position.x = pos.x;
		m_ControllerDesc.position.y = pos.y;
		m_ControllerDesc.position.z = pos.z;
