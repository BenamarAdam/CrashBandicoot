...
void SpriteRenderer::UpdateBuffer(const SceneContext& sceneContext)
{
	...
	if (m_pVertexBuffer)
	{
		...
		std::memcpy(subResource.pData, m_Sprites.data(), sizeof(VertexSprite) * m_BufferSize);
...
void SpriteRenderer::Draw(TextureData* pTexture, XMFLOAT2 position, XMFLOAT4 color,
	XMFLOAT2 pivot, XMFLOAT2 scale, float rotation, float depth)
{
	VertexSprite vertex{};
